...
m.df |> 
  ...
  geom_histogram() +
  facet_grid(island_dist_category~island_size_category)
...
scale_fill <- scale_fill_manual(
  ...
  values = c( "Mandarte Island"   = "black"      ,  
              ...
              "Saltspring Island" = "red4"       ,  
              "Portland Island"   = "green4"    ,   
              "South Pender"     = "orchid" ),  drop = FALSE ) 
...
m.df |> 
  ggplot(aes(y=sampling_site, x=weight_total_grams, 
             fill=sampling_site, color=sampling_site))+
  geom_boxplot()+
  scale_color+
    scale_fill
